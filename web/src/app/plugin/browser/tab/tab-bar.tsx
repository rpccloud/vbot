...
export class TabBar {
    ...
    public constructor(
        onChange: (focus: number, list: Array<PluginProps>) => void
    ) {
    ...
    public flush(
        ...
        if (isCalculate) {
            ...
            const showMoved = range(
                Math.floor(netWidth / minNetMoved),
                0,
                movedLen
            );
            const netMovedWidth =
                showMoved > 0 && showMoved === movedLen
    ...
    public addTab(
        ...
        if (param.kind === "home") {
            ...
            if (afterId === 0) {
                ...
                let movIndex = this.moved.findIndex(
                    (o) => o.getParam().tabID === param.tabID
                );
                if (movIndex < 0) {
    ...
    public deleteTab(pageId: number): boolean {
        let tab: Tab | undefined = this.moved.find(
            (o) => o.getParam().tabID === pageId
        );

        if (pageId === 0 && (tab = this.home)) {
            this.home = undefined;
        } else if (!!tab) {
            this.moved = this.moved.filter(
                (o) => o.getParam().tabID !== pageId
            );
        } else {
            return false;
    ...
    public setTabX(tab: Tab, x: number): boolean {
        if (tab.isHome()) {
            ...
            if (tab.index !== index) {
                this.moved = this.moved.filter(
                    (o) => o.getParam().tabID !== tab.getParam().tabID
                );
